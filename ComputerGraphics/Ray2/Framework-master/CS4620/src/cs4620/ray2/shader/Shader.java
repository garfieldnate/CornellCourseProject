...
public abstract class Shader {
	...
	protected double fresnel(Vector3d normal, Vector3d outgoing, double refractiveIndex) {
		...
		double theta1 = 0;
		double theta2 = 0;
		//Ray starts in the air
		if(outgoing.dot(normal) > 0){
			theta1 = outgoing.angle(normal);
			theta2 = Math.asin(Math.sin(theta1) / refractiveIndex);
		}
		//Ray starts in the glass	
		else{
			normal.negate();
			theta1 = outgoing.angle(normal);
			theta2 = Math.asin(Math.sin(theta1) * refractiveIndex);
		}
		double fp = (refractiveIndex * Math.cos(theta1) - Math.cos(theta2)) / (refractiveIndex * Math.cos(theta1) + Math.cos(theta2));
		double fs = (Math.cos(theta1) - refractiveIndex * Math.cos(theta2)) / (Math.cos(theta1) + refractiveIndex * Math.cos(theta2));
