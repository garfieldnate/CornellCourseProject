...
public class Bvh implements AccelStruct {   
	...
	private boolean intersectHelper(BvhNode node, IntersectionRecord outRecord, Ray rayIn, boolean anyIntersection)
	{
		...
		if(node.isLeaf()){
			...
			if(intersectHelper(node.child[0], ir, rayIn, anyIntersection)){
				outRecord.set(ir);
				ret = true;
			}
			if(intersectHelper(node.child[1], ir1, rayIn, anyIntersection)){
				if(!ret)
					outRecord.set(ir1);
				else
					if(ir1.t < ir.t)
	...
	private BvhNode createTree(int start, int end) {
		...
		double widX = maxB.x - minB.x;
		double widY = maxB.y - minB.y;
		double widZ = maxB.z - minB.z;
		int widestDim = -1;
		if(Math.max(Math.max(widY, widZ), widX) == widX)
			widestDim = 0;
		if(Math.max(Math.max(widY, widZ), widX) == widY)
			widestDim = 1;
		if(Math.max(Math.max(widY, widZ), widX) == widZ)
			widestDim = 2;
		...
		return new BvhNode(minB, maxB, createTree(start, (end + start) / 2), createTree((end + start) / 2, end), start, end);
	}
