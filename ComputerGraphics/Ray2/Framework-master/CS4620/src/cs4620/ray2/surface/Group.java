...
public class Group extends Surface {
  ...
  public void setTransformation(Matrix4d pMat, Matrix4d pMatInv, Matrix4d pMatTInv) {
    ...
    // i.e. The child's transformation will be applied to objects before its parent's.
	  	if(objs == null)
	  		return;
	  	Matrix4d temp = pMat.clone().mulBefore(transformMat);
	  	Matrix4d tempInv = temp.clone().invert();
	  	Matrix4d tempTInv = temp.clone().transpose().invert();
	  	tMat = temp;
    	tMatInv = tempInv;
    	tMatTInv = tempTInv;
	    //Modify all the surface transformation
	  	for(Surface object : objs){
	    // TODO#A7: Call setTransformation(tMat, tMatInv, tMatTInv) on each of the children.
		object.setTransformation(this.tMat, this.tMatInv, this.tMatTInv);
		//computeBoundingBox();
	    }
