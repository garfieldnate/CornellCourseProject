...
public class Triangle extends Surface {
	...
	public void computeBoundingBox() {
		...
		//Get the coordinates of triangle vertexes
		Vector3d v0 = tMat.mulPos(owner.getPosition(index.x));
		Vector3d v1 = tMat.mulPos(owner.getPosition(index.y));
		Vector3d v2 = tMat.mulPos(owner.getPosition(index.z));
		
		//Compute the bound coordinates
		double xMin = Math.min(v0.x, Math.min(v1.x, v2.x));
		double xMax = Math.max(v0.x, Math.max(v1.x, v2.x));
		double yMin = Math.min(v0.y, Math.min(v1.y, v2.y));
		double yMax = Math.max(v0.y, Math.max(v1.y, v2.y));
		double zMin = Math.min(v0.z, Math.min(v1.z, v2.z));
		double zMax = Math.max(v0.z, Math.max(v1.z, v2.z));
		
	    //Transform the bound coordinates into world space
	    Vector3d tempMin = new Vector3d(xMin, yMin, zMin);
	    //System.out.println(tempMin);
	    Vector3d tempMax = new Vector3d(xMax, yMax, zMax);
	    minBound = tempMin;
	    maxBound = tempMax;
