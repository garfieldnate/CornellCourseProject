...
public class CameraController {
	...
	private void rotate(Matrix4 parentWorld, Matrix4 transformation, Vector3 rotation) {
		...
		if(!orbitMode){
			//rotation.x first
			transformation.mulBefore(Matrix4.createRotationX((float)(rotation.x / 180 * Math.PI)));
			//rotation.y second
			transformation.mulBefore(Matrix4.createRotationY((float)(rotation.y / 180 * Math.PI)));
			//rotation.z third
			transformation.mulBefore(Matrix4.createRotationZ((float)(rotation.z / 180 * Math.PI)));
		}
		
		//orbit mode
		else{
			transformation.mulAfter(parentWorld.clone());
			//rotation.x first
			transformation.mulAfter(Matrix4.createRotationX((float)(rotation.x / 180 * Math.PI)));
			//rotation.y second
			transformation.mulAfter(Matrix4.createRotationY((float)(rotation.y / 180 * Math.PI)));
			//rotation.z third
			transformation.mulAfter(Matrix4.createRotationZ((float)(rotation.z / 180 * Math.PI)));
	...
	private void translate(Matrix4 parentWorld, Matrix4 transformation, Vector3 motion) {
		...
		Matrix4.createTranslation(motion, translate);
		transformation.mulBefore(translate);
	}
}
