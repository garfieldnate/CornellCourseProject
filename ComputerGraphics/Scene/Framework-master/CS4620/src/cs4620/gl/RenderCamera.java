...
public class RenderCamera extends RenderObject {
	...
	public void updateCameraMatrix(Vector2 viewportSize) {
		...
		if (this.sceneCamera.isPerspective){
			morth = Matrix4.createPerspective((float)this.sceneCamera.imageSize.x, (float)this.sceneCamera.imageSize.y, (float)this.sceneCamera.zPlanes.x, (float)this.sceneCamera.zPlanes.y);
		}
		//Compute projection matrix
		else{
			Matrix4.createOrthographic((float)sceneCamera.imageSize.x, (float)sceneCamera.imageSize.y, (float)sceneCamera.zPlanes.x, (float)sceneCamera.zPlanes.y, morth);
		}
		Matrix4 ratio = new Matrix4();
	    Matrix4.createScale((float) (viewportSize.y / viewportSize.x), 1, 1, ratio);
	    morth.mulAfter(ratio);
		this.mViewProjection.set(mWorldTransform).invert().mulAfter(morth);
