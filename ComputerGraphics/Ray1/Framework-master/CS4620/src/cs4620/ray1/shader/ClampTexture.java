...
public class ClampTexture extends Texture {
	...
	public Colord getTexColor(Vector2d texCoord) {
		...
		int i, j;
		i = Math.max(0, Math.min(getImage().getWidth() - 1,(int)(texCoord.x * getImage().getWidth() + 0.5)));
		j = Math.max(0, Math.min(getImage().getHeight() - 1, getImage().getHeight() - (int)(texCoord.y * getImage().getHeight() + 0.5)));
		
		//Convert color
		Color c = new Color(Color.fromIntRGB(getImage().getRGB(i, j)));
		Colord outputColor = new Colord(c);
		return outputColor;
	}
